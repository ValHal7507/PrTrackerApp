...
└── app/
    ...
    └── src/main/
        ...
        └── java/com/example/prtracker/
            ...
            │   ├── Pet.kt                   # Pet data class, PetRarity (10 tiers incl. DIVINE, SUPER, EXCLUSIVE, SECRET), PetTier (6 tiers), PetSpecies, PetCatalog, MiniGameSettings, RollResult
            │   ├── PetUpgrade.kt            # PetUpgrade enum (LUCK, ROLL_SPEED, LUCKY_ROLL, COIN_MULTIPLIER, EQUIP_SLOTS, MULTI_ROLL) with cost formulas
            │   ├── SpecialDice.kt           # SpecialDiceType enum (11 dice types incl. SUPER_DICE, GILDED_DICE, 4 BURST), DiceCategory (LUCK/TIER/BURST), SpecialDice, ActiveDiceEffect, SpecialDiceCatalog
...
data class Pet(
    ...
    val name: String = "",
    val rarity: String = "COMMON",    // COMMON, UNCOMMON, RARE, EPIC, LEGENDARY, MYTHICAL, DIVINE, SUPER, EXCLUSIVE, SECRET
...
`PetRarity` enum defines 10 rarity tiers with drop chances, base coin values, and base XP multipliers: COMMON (54.4%, 100 coins, 1.00x), UNCOMMON (28%, 250 coins, 1.05x), RARE (13%, 600 coins, 1.10x), EPIC (4%, 1500 coins, 1.20x), LEGENDARY (0.5%, 5000 coins, 1.35x), MYTHICAL (0.1%, 15000 coins, 1.50x), DIVINE (0.05% each 1/2000, 50000 coins, 1.60x — species-specific overrides), SUPER (0.001% 1/100k independent, 100B coins, 1.00x — dynamic formula), EXCLUSIVE (1/1M independent, 100T coins, 2.0x — dynamic formula), SECRET (1/2M independent, 5Q coins, 1.0x — dynamic formula).

`PetTier` enum defines 6 evolution tiers with coin multipliers and XP multipliers: NORMAL (1x, 1.00x), SILVER (2x, 1.10x), GOLDEN (4x, 1.25x), RAINBOW (8x, 1.45x), DARK_MATTER (16x, 1.70x), RED_MATTER (32x, 2.00x). A pet's coin value = `PetRarity.baseCoins × PetTier.coinMultiplier × stars` (SUPER/EXCLUSIVE/SECRET skip star multiplier). A pet's XP multiplier = `PetRarity.baseXpMult × PetTier.xpMult × (1 + (stars-1) × 0.05f)` (species-specific override if `PetSpecies.xpMult` is set). SUPER pets: `1.1f × tier.xpMult × bestNonSuperMultiplier`. EXCLUSIVE pets: `2.0f × tier.xpMult × bestNonSuperMultiplier`. SECRET pets: `5.0f × tier.xpMult × bestNonPremiumMultiplier`.

`PetSpecies` holds 25 species (5 COMMON, 4 UNCOMMON, 3 RARE, 2 EPIC, 2 LEGENDARY, 2 MYTHICAL, 3 DIVINE, 2 SUPER, 1 EXCLUSIVE, 2 SECRET). `PetCatalog.allSpecies` is the registry. Species are code-only — only `Pet` instances are persisted to JSON. DIVINE species have per-species XP overrides via `PetSpecies.xpMult`: Aurelion (1.60x), Seraphel (1.75x), Thundius (2.00x).

`fun Pet.coinValue()` extension function computes the sell value. `fun Pet.xpMultiplier(inventory)` extension function computes XP multiplier with dynamic SUPER/EXCLUSIVE/SECRET formulas. Fusion consumes a 5★ pet and creates the next tier with 1★. Premium fusion (SUPER/EXCLUSIVE/SECRET) consumes 3 same-species+tier pets and creates 1★ of the next tier.

`data class RollResult(pet, effectiveChances, isLuckyRoll, wasSold, displayOneInX)` — returned by `rollDice()`. `effectiveChances` contains the actual boosted chances used for the roll (including lucky roll boost), used for the "1 in X CHANCE" display on pet reveal. `displayOneInX` stores the trigger probability when a DIVINE override fires (e.g. 1/9 from MYTHIC dice), used for accurate display.

`data class MiniGameSettings(autoSellRarities, freezeRarities, selectedRollCount)` — per-user minigame preferences stored in `PetStorageData.miniGameSettings`. `autoSellRarities` is a `Set<String>` of rarity names to auto-sell on roll (SECRET always excluded). `freezeRarities` is a `Set<String>` of rarity names that trigger the 5-second premium reveal freeze (default: EXCLUSIVE, SECRET). `selectedRollCount` is the chosen dice count per roll (0 = use max unlocked).
...
data class PetStorageData(
    ...
    val miniGameSettings: MiniGameSettings = MiniGameSettings(),  // Auto-sell preferences + freeze preferences + roll count selector
...
| `loadPetData(): PetStorageData`                                                                                                                                      | Reads `pets.json` and returns the full `PetStorageData` object. Falls back to migrating pet fields from old `prs.json` via `migrateIfNeeded()`. Runs species ID renames (e.g. `dumbbell_01` → `biceps_01`) for backward compatibility. Runs `collapseDiceInventory()` to merge duplicate dice entries. |
...
- **Premium reveal freeze:** When any rolled pet's rarity is in the `freezeRarities` set (default: EXCLUSIVE, SECRET), a 5-second freeze blocks both dice tap and auto-roll. A `premiumFreezeSeconds` countdown decrements every 1000ms. The freeze is silent — no countdown text is shown. Configurable via the Freeze Screen section in MiniGameSettingsScreen.
- **displayOneInX override:** When a DIVINE trigger fires (e.g. 1/9 from MYTHIC dice), the `RollResult.displayOneInX` field stores the trigger probability. `RevealView` uses this override instead of computing from effective chances, showing accurate "1 in X CHANCE" for trigger-based drops.
- **Premium pet handling (SUPER/EXCLUSIVE/SECRET):** `RevealView` applies premium treatment: larger emoji (56sp vs 48sp), pulsing glow + horizontal shake animation, no stars display, and special labels — "LEGENDARY DISCOVERY" for SUPER, "BEYOND DIVINE" for EXCLUSIVE, "🔓 ULTRA SECRET 🔓" for SECRET. `MultiRevealView` uses 100dp boxes (vs 80dp normal) for premium pets.
- **PET_DETAIL state:** Full-screen `PetDetailView` overlay (bottom dice area hidden) showing:
  ...
  - Rarity badge pill (color-coded, "SUPER" for SUPER, "EXCLUSIVE" for EXCLUSIVE, "SECRET" for SECRET)
  - Tier badge pill (color-coded with tier label)
  - Stars display (★/☆, gold when >= 3 stars; hidden for SUPER, EXCLUSIVE, SECRET)
  - **Favorite toggle** — star icon button, toggles `isFavorited` with haptic feedback
  - **FUSE button** (visible when 5★ and not max tier, not SUPER/EXCLUSIVE/SECRET) — upgrades pet to next tier, consumes old pet, resets stars to 1. Calls `viewModel.fusePet(petId)`.
  - **SELL button** — sells pet for coin value (baseCoins × tierMultiplier × stars for normal; raw coinValue for SUPER/EXCLUSIVE/SECRET). Shows sell value with `formatCoin1(value)`. Calls `viewModel.sellPet(petId)`. Shows confirmation dialog before selling.
...
- **Pet grid:** `LazyVerticalGrid` with 4 columns showing all owned pets sorted by favorites first, then by selected sort mode. Each `PetCollectionCard` shows emoji + pet name (truncated) with tier-specific visual effects:
  ...
  - Premium pets (SUPER/EXCLUSIVE/SECRET) get pulsing glow + horizontal shake, no stars display
...
- **Premium fuse mode:** ⚡ button in header (visible when 3+ same-species+tier premium pets exist). Selecting enters premium fuse mode — mutually exclusive with sell select mode. Max 3 selections. Bottom bar shows selected pets with arrow to output tier. "FUSE ⚡" button triggers confirmation dialog. Calls `viewModel.fusePremiumPets()`. Consumes 3 pets, creates 1★ of next tier.
- **Action buttons row:** FUSE ALL and SELL ALL buttons at the bottom (outside grid, fixed). Each has a confirmation dialog. Hidden when premium fuse mode is active.
- **Merge import:** MERGE mode deduplicates pets by ID, keeps higher tier then higher stars, `maxOf` for coins.
...
- **Auto-Sell section:** `GlowingCard` containing a list of rarity checkboxes. Each checkbox shows a colored rarity dot + rarity name + "AUTO SELL" label. SECRET is always excluded (always kept). EXCLUSIVE appears in the checkbox list (auto-sellable). Checked rarities are stored in `_miniGameSettings.autoSellRarities`. Description text below: "Sold on roll — still shown in showcase".
- **Freeze Screen section:** `GlowingCard` containing a list of rarity checkboxes for the premium reveal freeze. Each checkbox shows a colored rarity dot + rarity name. All 10 rarities are shown. Default: EXCLUSIVE and SECRET checked. Checked rarities stored in `_miniGameSettings.freezeRarities`. Controls which rarities trigger the 5-second freeze on roll.
...
- **Species grid:** `LazyVerticalGrid` with 4 columns showing all 25 PetCatalog species for the selected tier. Each cell shows:
...
- **Pet detail overlay:** Tapping an unlocked species opens a full-screen overlay showing:
  ...
  - Star management (non-SUPER/EXCLUSIVE/SECRET only): interactive ★/☆ selector, sell button with coin value, equip/unequip button
  - SUPER pets show tier pills and formula display ("1.1 × TIER × BEST_NON_SUPER")
  - EXCLUSIVE pets show tier pills and formula display ("2.0 × TIER × BEST_NON_SUPER")
  - SECRET pets show tier pills and formula display ("5.0 × TIER × BEST_NON_SUPER")
  - `bestNonSuperMult` excludes SUPER, EXCLUSIVE, and SECRET from the lookup
...
| `totalRolls`         | `StateFlow<Int>`                                  | Lifetime dice roll count                                                                                                                                                       |
| `rollsSinceEpicOrAbove` | `StateFlow<Int>`                               | Soft pity counter: Epic+ chance +1% per roll after 150 rolls                                                                                                                  |
| `rollsSinceLegendary` | `StateFlow<Int>`                                | Hard pity: guaranteed legendary at 401 rolls                                                                                                                                   |
| `rollsSinceMythical` | `StateFlow<Int>`                                 | Hard pity: guaranteed mythical at 2001 rolls                                                                                                                                   |
| `rollsSinceDivine`   | `StateFlow<Int>`                                 | Hard pity: guaranteed divine at 5001 rolls                                                                                                                                     |
...
| `diceInventory`      | `StateFlow<List<SpecialDice>>`                   | Owned special dice (consumable), persisted to pets.json                                                                                                                        |
| `activeDiceEffects`  | `StateFlow<List<ActiveDiceEffect>>`              | Active dice queue (strongest first), persisted to pets.json                                                                                                                    |
...
| `miniGameSettings`   | `StateFlow<MiniGameSettings>`                     | Auto-sell preferences, freeze preferences, and roll count selector, persisted to pets.json                                                                                     |
...
| `rollDice(): Pet`                                    | Multi-tier weighted random rarity selection with pity system. **Branch structure:** (1) SUPER DICE path: 1/200k → SECRET (flat 1Q × tierMult coins), 1/1k → EXCLUSIVE, else → guaranteed SUPER (50B flat). All reset pity. (2) Natural EXCLUSIVE: 1/1M flat check (MYTHIC dice: 1/100k). (3) Natural SUPER: 1/100k flat check, 50B flat coins. (4) Natural SECRET: 1/2M flat check, 1Q × tierMult coins. (5) Weighted roll: base chances from dice `baseChances` or rarity dropChance; luck ×20%/level; soft pity at 150 (Epic+); hard pity at 401 (Legendary), 2001 (Mythical), 5001 (Divine). Active dice: REFINING/ASCENDANT/LEGENDARY use custom distributions; BANISHING/MYTHIC use min/max filter. All dice: 1/9 (MYTHIC), 1/1000 (BANISHING), 1/100 (REFINING), 1/20 (ASCENDANT), 1/8 (LEGENDARY) DIVINE override chance. Lucky Roll: every 5th roll boosted. SUPER/EXCLUSIVE/SECRET coins: raw `pet.coinValue()` (no multipliers). Normal coins: `pet.coinValue() × upgradeMult × petCoinMult`. Auto-sell: SECRET always excluded. Returns `RollResult(pet, effectiveChances, isLuckyRoll, displayOneInX)`. Saves to JSON. |
...
| `sellPet(petId: String)`                             | Removes pet by ID, adds `pet.coinValue()` to coins. Premium pets (SUPER/EXCLUSIVE/SECRET) get raw coinValue (no coin multiplier). Normal pets get `coinValue × coinMultiplier`. Removes pet ID from equipped list. Saves to pets.json.                                                                                                                                                                                                                                                                                                                          |
...
| `rollDiceMultiple(count: Int)`                        | If a BURST dice effect is active: consumes up to 100k rolls per tap (`minOf(rollsRemaining, 100_000)`), updates the BURST effect's `rollsRemaining` in-place when rolls remain (only removes when count hits 0), calls `burstRoll(rollCount)`, returns single-element list. Otherwise: calls `rollDice()` `count` times, collecting results into `multiRollResults`. Sets `selectedResultIndex = 0`. Used for MULTI_ROLL upgrade. Saves to pets.json once after batch. |
...
| `burstRoll(rollCount: Int): RollResult`               | Executes N rolls instantly without coins, tracks best pet (by rarity ordinal → tier ordinal → stars), applies proportional LUCK/TIER consumption via local counters, returns single `RollResult` with best pet. Called by `rollDiceMultiple()` when a BURST effect is active. |
| `setAutoSellRarity(rarity, enabled)`                  | Adds or removes rarity from `_miniGameSettings.autoSellRarities`. Saves to pets.json. EXCLUSIVE is now auto-sellable (only SECRET is always excluded). |
| `setFreezeRarity(rarity, enabled)`                    | Adds or removes rarity from `_miniGameSettings.freezeRarities`. Saves to pets.json. Controls which rarities trigger the 5-second premium reveal freeze.                                                                                                                                                                                                                                                                                                                                                                                                          |
| `removeActiveDiceEffect(typeId)`                      | Removes a dice effect by typeId from `_activeDiceEffects`. Called when user manually removes an active dice chip (2-step confirmation, no refund). Saves to pets.json.                                                                                                                                                                                                                                                                                                                                                                                            |
| `equipBest()`                                         | Sorts inventory by xpMultiplier descending, takes maxSlots, sets equippedPetIds. Convenience function for auto-equipping optimal pets.                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `coinMultiplier(): Float`                             | Returns `1.0f + getUpgradeLevel(COIN_MULTIPLIER) * 0.20f`. Used in sellPet/sellAllUnfavorited for non-premium coin calculation.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `fusePremiumPets(ids: List<String>): Pet?`            | Takes exactly 3 pet IDs. Validates all are SUPER/EXCLUSIVE/SECRET, same speciesId, same tier, and next tier exists. Creates new Pet at nextTier with stars=1. Consumes all 3 input pets. Removes from equipped list. Saves to pets.json. Returns the fused pet or null on validation failure.                                                                                                                                                                                                                                                                       |
...
42. **DIVINE Rarity** — Added DIVINE rarity tier (ordinal 6, between MYTHICAL and SUPER): 0.05% drop chance each (1/2000), 50,000 base coins, 1.60x base XP mult. Three species: Aurelion 👑 (1.60x XP), Seraphel 🌟 (1.75x XP), Thundius ⚡ (2.00x XP). Color 0xFFFFE44D. Added `xpMult: Float?` field to `PetSpecies` for per-species XP override. `Pet.xpMultiplier()` checks `species.xpMult` before falling back to `rarity.baseXpMult`. Added `rollsSinceDivine` pity counter to PetStorageData with hard pity at 5001 rolls. DIVINE trigger fires on all dice: 1/9 (MYTHIC), 1/1000 (BANISHING), 1/100 (REFINING), 1/20 (ASCENDANT), 1/8 (LEGENDARY). Trigger zeroes all rarities and sets DIVINE=1.0. Added `displayOneInX` field to `RollResult` for accurate "1 in X CHANCE" display when DIVINE trigger fires.

43. **EXCLUSIVE Rarity** — Added EXCLUSIVE rarity tier (ordinal 8, above SUPER): 1/1M natural drop chance, 100T base coins, 2.0x base XP mult. One species: Zenith 🐉. Color 0xFF7B35C1. No pity counter. XP formula: `2.0 × tier.xpMult × bestNonSuperMultiplier` (excludes SUPER/EXCLUSIVE/SECRET from best lookup). SUPER DICE gives 1/1k chance of EXCLUSIVE. MYTHIC dice gives 1/100k chance of EXCLUSIVE. Neither SUPER nor EXCLUSIVE have stars — both hidden in UI. Premium fusion supported (3 same-species+tier → next tier). Auto-sell excluded (always kept). Premium reveal freeze applies.

44. **SECRET Rarity** — Added SECRET rarity tier (ordinal 9, rarest): 1/2M natural drop chance, 5Q base coins, 1.0x base XP mult. Two species: Ozzy 🦊 (Fox Terrier), Biceps 💪. Color 0xFF000000 (black). No pity counter. XP formula: `5.0 × tier.xpMult × bestNonPremiumMultiplier` (excludes SUPER/EXCLUSIVE/SECRET from best lookup). SUPER DICE gives 1/200k chance of SECRET. Roll reward: 1Q × tierMultiplier. No stars. Premium fusion supported. Auto-sell excluded (always kept). Premium reveal freeze applies. "🔓 ULTRA SECRET 🔓" label in reveal.

45. **Premium Pet Fusion** — Added `fusePremiumPets(ids: List<String>)` to PRViewModel. Takes exactly 3 pet IDs. Validates all are SUPER/EXCLUSIVE/SECRET, same speciesId, same tier, and next tier exists. Creates new Pet at nextTier with stars=1. Consumes all 3 input pets. Added premium fuse mode to PetInventoryScreen: ⚡ button in header (visible when 3+ same-species+tier premium pets exist), selection UI (max 3), bottom bar with output tier preview, confirmation dialog. Mutually exclusive with sell select mode. FUSE ALL and SELL ALL buttons hidden when premium fuse mode is active.

46. **SUPER Auto-Sell** — SUPER pets are now auto-sellable like Divine and lower. Only EXCLUSIVE and SECRET are always-protected from auto-sell. Updated rollDice() auto-sell gate, sellPet(), sellAllUnfavorited(), sellPets(), and PetInventoryScreen sell displays to treat SUPER as non-premium for coin calculation (coin multiplier upgrade DOES affect SUPER sell value). SUPER roll reward remains flat 50B (no multipliers).

47. **Premium Reveal Freeze** — Added 5-second freeze when rolling any pet whose rarity is in the `freezeRarities` set (default: EXCLUSIVE, SECRET). Blocks both dice tap and auto-roll during freeze. Configurable via FREEZE SCREEN section in MiniGameSettingsScreen with per-rarity checkboxes for all 10 rarities. `MiniGameSettings.freezeRarities: Set<String>` persisted to pets.json.

48. **Qd Coin Formatting** — Added quadrillion (Qd) to all `formatCoins`/`formatCoin1` functions across DiceRollScreen, PetInventoryScreen, PetUpgradesScreen, DiceShopScreen. Threshold: `1_000_000_000_000_000L`. Used for SECRET base sell value (5Qd) and roll reward (1Qd).

49. **Burst Dice System** — Added 4 burst dice types to `SpecialDice.kt`: BURST_100 (1M), BURST_1000 (10M), BURST_10000 (1B), BURST_100000 (10T). Added `DiceCategory.BURST` enum value. Burst dice activate via `useDiceByType()` creating an `ActiveDiceEffect(category=BURST)`. When user taps roll, `rollDiceMultiple()` detects the BURST effect and calls `burstRoll(count)` — executes all rolls instantly, awards no coins, tracks best pet (rarity→tier→stars), returns single `RollResult`. BURST dice have their own queue separate from LUCK and TIER. Multiple burst dice stack in queue (fire in sequence). Added `isBurstDice` property and updated `strengthOrder`.

50. **EXCLUSIVE Auto-Sell** — EXCLUSIVE rarity is now auto-sellable like Divine and lower. Only SECRET remains always-protected from auto-sell. Updated `rollDice()` auto-sell gate, `sellPet()`, `sellAllUnfavorited()`, `sellPets()`, and `PetInventoryScreen` sell displays. Added EXCLUSIVE checkbox to `MiniGameSettingsScreen` auto-sell section.

51. **Qd → Q Coin Formatting** — Changed all `formatCoins`/`formatCoin1` functions from `"Qd"` to `"Q"` (5 occurrences across 4 files). Cleaner label for quadrillion.

52. **DiceShopScreen MAX Button Overflow Fix** — Fixed integer overflow when MAX button was tapped with large coin balance. Changed `maxQty` from `Int` to `Long`, added safe clamping at the call site via `buyDice()`.
