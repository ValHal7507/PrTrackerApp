...
enum class PetUpgrade(
    ...
    fun costForLevel(currentLevel: Int): Long {
        ...
        return (baseCost * (currentLevel + 1).toDouble().pow(1.7)).toLong()
    }
...
31. **Upgrade Cost** — Formula changed multiple times: exponential `costMultiplier^level` → dampened `costMultiplier^(level × 0.82f)` → power `baseCost × (level+1)^1.7` → recursive `baseCost × 1.10^level` → `baseCost × 1.25^level` (standard) and `baseCost × 1.8^level` (ROLL_SPEED). ROLL_SPEED max level set to 23 (0ms delay) with `maxLevel()` and `costForLevel()` overrides. Standard upgrades reach trillions at L100+; ROLL_SPEED costs ~279M to max. Multipliers: LUCK 1.12x, COIN_MULTIPLIER 1.13x, ROLL_SPEED 1.10x, LUCKY_ROLL 1.15x.
