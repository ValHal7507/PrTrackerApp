...
`data class MiniGameSettings(autoSellRarities, freezeRarities, selectedRollCount)` — per-user minigame preferences stored in `PetStorageData.miniGameSettings`. `autoSellRarities` is a `Set<String>` of rarity names to auto-sell on roll (EXCLUSIVE and SECRET always excluded). `freezeRarities` is a `Set<String>` of rarity names that trigger the 5-second premium reveal freeze (default: EXCLUSIVE, SECRET). `selectedRollCount` is the chosen dice count per roll (0 = use max unlocked).
...
- **Auto-Sell section:** `GlowingCard` containing a list of rarity checkboxes. Each checkbox shows a colored rarity dot + rarity name + "AUTO SELL" label. EXCLUSIVE and SECRET are always excluded (always kept). SUPER appears in the checkbox list (auto-sellable). Checked rarities are stored in `_miniGameSettings.autoSellRarities`. Description text below: "Sold on roll — still shown in showcase".
...
| `rollDice(): Pet`                                    | Multi-tier weighted random rarity selection with pity system. **Branch structure:** (1) SUPER DICE path: 1/200k → SECRET (flat 1Qd × tierMult coins), 1/1k → EXCLUSIVE, else → guaranteed SUPER (50B flat). All reset pity. (2) Natural EXCLUSIVE: 1/1M flat check (MYTHIC dice: 1/100k). (3) Natural SUPER: 1/100k flat check, 50B flat coins. (4) Natural SECRET: 1/2M flat check, 1Qd × tierMult coins. (5) Weighted roll: base chances from dice `baseChances` or rarity dropChance; luck ×20%/level; soft pity at 150 (Epic+); hard pity at 401 (Legendary), 2001 (Mythical), 5001 (Divine). Active dice: REFINING/ASCENDANT/LEGENDARY use custom distributions; BANISHING/MYTHIC use min/max filter. All dice: 1/9 (MYTHIC), 1/1000 (BANISHING), 1/100 (REFINING), 1/20 (ASCENDANT), 1/8 (LEGENDARY) DIVINE override chance. Lucky Roll: every 5th roll boosted. SUPER/EXCLUSIVE/SECRET coins: raw `pet.coinValue()` (no multipliers). Normal coins: `pet.coinValue() × upgradeMult × petCoinMult`. Auto-sell: EXCLUSIVE and SECRET always excluded. Returns `RollResult(pet, effectiveChances, isLuckyRoll, displayOneInX)`. Saves to JSON. |
...
| `setAutoSellRarity(rarity, enabled)`                  | Adds or removes rarity from `_miniGameSettings.autoSellRarities`. Saves to pets.json. EXCLUSIVE and SECRET are always excluded (always kept).                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
44. **SECRET Rarity** — Added SECRET rarity tier (ordinal 9, rarest): 1/2M natural drop chance, 5Qd base coins, 1.0x base XP mult. Two species: Ozzy 🦊 (Fox Terrier), Dumbbell 🔩. Color 0xFF000000 (black). No pity counter. XP formula: `5.0 × tier.xpMult × bestNonPremiumMultiplier` (excludes SUPER/EXCLUSIVE/SECRET from best lookup). SUPER DICE gives 1/200k chance of SECRET. Roll reward: 1Qd × tierMultiplier. No stars. Premium fusion supported. Auto-sell excluded (always kept). Premium reveal freeze applies. "🔓 ULTRA SECRET 🔓" label in reveal.
