...
└── app/
    ...
    └── src/main/
        ...
        └── java/com/example/prtracker/
            ...
            ├── data/
            │   ├── Exercise.kt               # Exercise data class (with isPinned, sortOrder)
...
- **Format:** Gson-serialized `StorageData` object (top-level object with `exercises`, `goals`, `weightEntries`, `settings`, `restDays`, `runEntries`, `runningPRs`, `workoutPresets`, `workoutSession`, and `workoutHistory` fields)
...
| `saveFullData(exercises, goals, weightEntries, settings, restDays, runEntries, runningPRs, workoutPresets, workoutSession, workoutHistory)` | Saves the full `StorageData` object (all ten fields: exercises, goals, weight entries, settings, restDays, run entries, running PRs, workout presets, workout session, and workout history). |
...
- **Name field:** `OutlinedTextField` with cyan cursor, cyan focus border, dark container.
...
- **Goal field:** Numeric `OutlinedTextField` for optional target. Label: "Set a target to track progress (optional)". _The goal value is accepted but currently **not stored or used anywhere**._
- **Confirm button:** `NeonButton` with text "CONFIRM". Disabled if name is blank.
  - On click: generates UUID, creates `Exercise(name, type, emptyList(), sortOrder = maxOrder + 1)`, calls `viewModel.addExercise()`, pops back to Dashboard.
...
- **Title row:** Back arrow, exercise name, delete (trash) icon.
- **Line chart** (MPAndroidChart via `AndroidView`):
  ...
  - X axis: date labels (MM/dd format), Y axis: values
...
- **Meta card:** `GlowingCard` showing description, difficulty badge (color-coded), exercise count, and created-at date.
...
- **Row details:** Exercise name, value with reps/s suffix, formatted date ("MMM dd, yyyy HH:mm"), optional note. PR entries show a green trophy icon and "PR" label.
...
|---|---|
| `loadData()` | Reads full `StorageData` via `StorageManager.loadFullData()` and updates all 10 state flows (exercises, goals, weightEntries, appSettings, restDays, runEntries, runningPRs, workoutPresets, activeSession, workoutHistory) + syncs `SoundEngine.volume` |
| `saveData()` | Writes exercises + goals + weight entries to `StorageManager.saveData()` (private) |
| `addExercise(exercise)` | Appends with auto-incremented `sortOrder` within the same pin group + saves |
| `logEntry(exerciseId, entry)` | Appends entry to matching exercise, moves it to front of list + saves |
| `deleteExercise(exerciseId)` | Removes exercise by ID + removes all associated goals + saves |
| `deleteEntry(exerciseId, entryId)` | Removes specific entry from exercise + saves |
| `clearAllData()` | Resets all 11 state flows to defaults (exercises, goals, weightEntries, appSettings, restDays, runEntries, runningPRs, workoutPresets, activeSession, workoutHistory, allTelemetry) + saves |
...
| `importSyncData(data: StorageData, mode: SyncMode)` | Merges or replaces local data with incoming `StorageData`. REPLACE mode overwrites all 9 state flows (exercises, goals, weight, settings, restDays, runEntries, runningPRs, presets, history). MERGE mode deduplicates exercises by ID/name, merges goals/weight/restDays/presets/history by ID, deduplicates run entries by ID and recomputes running PRs. |
...
| `finishWorkout()` | Logs each completed set as a separate `PREntry` (note: "From workout: {presetName}") to the matching exercise, moves exercise to front, marks session completed, saves completed session to `_workoutHistory` + saves |
